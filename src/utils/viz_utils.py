...
def plot_graph_comparison(W_hat, A_true_list, save_path, title="Graph Recovery"):
    """
    ...
    Args:
        ...
        A_true_list: list[np.ndarray], each (D, D)
            Ground-truth lag matrices.
        save_path: str
    ...
    vmax = max(np.abs(W_hat).max(), np.max([np.abs(A).max() for A in A_true_list])) + 1e-6
    ...
    for lag in range(L):
        # True
        sns.heatmap(A_true_list[lag], ax=axes[0, lag], cmap="RdBu_r", center=0,
                    cbar=(lag==L-1), vmin=-vmax, vmax=vmax, annot=False)
